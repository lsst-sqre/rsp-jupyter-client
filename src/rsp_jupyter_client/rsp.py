...
__all__ = ["RSPJupyterClient", "JupyterLabSession"]
...
class JupyterLabSession:
    ...
    type should be created by calling `RSPJupyterClient.open_lab_session`.
...
def _convert_exception(
    f: Callable[Concatenate[RSPJupyterClient, P], Coroutine[None, None, T]],
) -> Callable[Concatenate[RSPJupyterClient, P], Coroutine[None, None, T]]:
    ...
    @wraps(f)
    async def wrapper(
        client: RSPJupyterClient, *args: P.args, **kwargs: P.kwargs
    ) -> T:
        try:
...
def _convert_iterator_exception(
    f: Callable[Concatenate[RSPJupyterClient, P], AsyncIterator[T]],
) -> Callable[Concatenate[RSPJupyterClient, P], AsyncIterator[T]]:
    """Convert web errors to a `~rsp_jupyter_client.JupyterWebError`.
...
class RSPJupyterClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._logger.debug("Created new RSPJupyterClient")
